...
from roman_numeral import RomaanNumeral
...
class Parser:
    ...
    def parse(cls, sentence):
        ...
        while len(tokens) > 1:
            ...
            if not right.isdigit():
                ...
                if operator == "-":
                    tree.append(
                        Subtract(left_expression, tree[-1])
                    )
                if operator == "+":
                    tree.append(
                        Add(left_expression, tree[-1])
                    )
            else:
                ...
                if not tree:
                    ...
                    if operator == "-":
                        tree.append(
                            Subtract(tree[-1], right_expression)
                        )
                    if operator == "+":
                        tree.append(
                            Add(tree[-1], right_expression)
                        )
        return tree.pop()
    ...
    def decide_left_expression(tree, tokens):
        ...
        # Every consecutive expression is reference to an existing AST row.
        
        left = tokens.pop(0)
        left_expression = None

        if not tree:        # only applicable if first round
            if not left.isdigit():      # if the 1st token is a roman numeral
